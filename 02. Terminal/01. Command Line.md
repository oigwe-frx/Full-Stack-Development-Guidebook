...
|---------|---------|------------| ------------|
| pwd | Print Working Directory | The shell command pwd displays the file path from the root directory to the current working directory. | Input ``` $ pwd ``` <br/> ```/Users/johnDoe/Desktop``` |
