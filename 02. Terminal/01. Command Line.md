...
| ls | List | The shell command ls is used to list the contents of a directory. If no arguments are given, it will list the contents of the current working directory. | Input: ```$ ls Desktop```  <br/> Output: ```job_application.pdf``` |
